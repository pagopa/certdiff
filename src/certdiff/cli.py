...
@click.command()
@click.option('--old', 'old_cert_path', type=click.Path(exists=True), help='Path to the old certificate PEM file.')
@click.option('--old-url', 'old_cert_url', help='URL to fetch the old certificate from a remote HTTPS endpoint.')
@click.option('--new', 'new_cert_path', required=True, type=click.Path(exists=True), help='Path to the new certificate PEM file.')
@click.option('--verbose', is_flag=True, help='Enable verbose output.')
@click.option('--report-json', 'report_file', default=None, help='Optional: Output a JSON report of the comparison.')
def main(old_cert_path, old_cert_url, new_cert_path, verbose, report_file):
    ...
    if differences:
        ...
        for diff in differences:
            click.echo(f"- {diff['field'].title()}: {diff['old']} ➔ {diff['new']}")
        raise SystemExit(1)
    else:
